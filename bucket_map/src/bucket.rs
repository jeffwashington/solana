use crate::index_entry::OccupyIfMatches;
...
pub struct Bucket<T: Copy + PartialEq + 'static> {
    ...
    /// true if this bucket was loaded (as opposed to created blank). When populating, we want to look for data on disk that already matches.
    look_for_existing: bool,
}
...
impl<'b, T: Clone + Copy + PartialEq + std::fmt::Debug + 'static> Bucket<T> {
    pub fn new(
        ...
        let (index, random) = restart
            ...
            .unwrap_or_else(|| {
                ...
                let (mut index, file_name) = BucketStorage::new(
                    Arc::clone(&drives),
                    1,
                    std::mem::size_of::<IndexEntry<T>>() as u64,
                    max_search,
    ...
    pub fn batch_insert_non_duplicates_internal(
        ...
        loop {
            ...
            if !look_for_existing {
                // loop completed while NOT looking for existing, so nothing more to be done.
                break;
            }
