...
pub struct BucketMap2<T: Clone + Copy + Debug> {
...
impl<T: Clone + Copy + Debug> Drop for BucketMap2<T> {
    fn drop(&mut self) {
        if self.temp_dir.is_none() {
            BucketMap2::<T>::erase_previous_drives(&self.drives);
        }
    }
}

impl<T: Clone + Copy + Debug> std::fmt::Debug for BucketMap2<T> {
...
impl<T: Clone + Copy + Debug> BucketMap2<T> {
