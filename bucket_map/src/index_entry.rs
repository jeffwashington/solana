...
impl<T: Copy + PartialEq + 'static> BucketOccupied for IndexBucketUsingBitVecBits<T> {
    fn occupy(&mut self, element: &mut [u8], ix: usize) {
        assert!(self.is_free(element, ix));
        // note that this leaves the entry with garbage data for the single index entry.
        // This state does not persist and the enum value will be overwritten shortly with the correct
        // value.
        // Previously, we were also incorrect by writing ZeroSlots. In the current impl, we always have a
        // &mut to the bucket, so no other thread will see this temporary state.
        // By choosing OneSlotInIndex here, we prevent ourselves from having to use the 2nd bit per entry for the enum.
        self.set_enum_tag(ix as u64, OccupiedEnumTag::OneSlotInIndex(T::default()));
    }
    fn free(&mut self, element: &mut [u8], ix: usize) {
        assert!(!self.is_free(element, ix));
...
impl<T: Copy + PartialEq + 'static> IndexEntryPlaceInBucket<T> {
    ...
    pub fn init(&self, index_bucket: &mut BucketStorage<IndexBucket<T>>, pubkey: &Pubkey) {
        ...
        // &mut to the bucket, so no other thread will see this temporary state.
        // By choosing OneSlotInIndex here, we prevent ourselves from having to use the 2nd bit per entry for the enum.
        self.set_slot_count_enum_value(index_bucket, OccupiedEnum::OneSlotInIndex(T::default()));
        let index_entry = index_bucket.get_mut::<IndexEntry<T>>(self.ix);
