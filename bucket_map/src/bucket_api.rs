...
type LockedBucket<T> = RwLock<Option<Bucket2<T>>>;
...
impl<T: Clone + Copy> BucketApi<T> {
    ...
    fn get_write_bucket(&self) -> RwLockWriteGuard<Option<Bucket2<T>>> {
        let mut bucket = self.bucket.write().unwrap();
        if bucket.is_none() {
            *bucket = Some(Bucket2::new(
