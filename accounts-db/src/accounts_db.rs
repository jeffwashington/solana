...
pub struct AccountsDb {
    ...
    pub(crate) uncleaned_pubkeys: DashMap<Slot, Vec<Pubkey>>,
...
pub(crate) struct ShrinkAncientStats {
    ...
    pub(crate) pubkeys_marked_for_clean: AtomicU64,
}
    ...
    pub(crate) fn report(&self) {
        datapoint_info!(
            ...
            (
                ...
                self.pubkeys_marked_for_clean.swap(0, Ordering::Relaxed) as i64,
                i64
            ),
        );
    }
                                                ...
                                                if !uncleaned_roots.is_empty() {
                                                    ...
                                                    if entries.len() > 1 {
                                                        ...
                                                        if let Some((last_slot, info)) = entries.last() {
                                                            let d = max_clean_root_inclusive.map(|m| *last_slot as i64 - m as i64).unwrap_or(i64::MAX);
                                                            if info.is_zero_lamport() && d > -10_000 && d < 10_000 {
    ...
    fn generate_index_for_slot(
        ...
        let items = accounts.map(|stored_account| {
            ...
            if pk == *pubkey || interesting.contains(pubkey) {
                log::error!("{pubkey}: {slot}, lamports: {}", stored_account.lamports());
            }
            if secondary {
