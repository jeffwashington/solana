...
impl AccountsDb {
    ...
    fn process_storage_slot<'a>(
        ...
        storage.accounts.account_iter().for_each(|stored_account| {
            ...
            if let Some(duplicate_account) = accounts_map.insert(*pubkey, stored_account) {
                storage_info_local.count_dead += 1;
                storage_info_local.stored_size_dead += duplicate_account.stored_size();
            }
        });
        accounts_map
    }
    ...
    fn generate_index_for_slot(
...

        {
            ...
            info.stored_size_dead += stored_size_dead;
    ...
    fn set_storage_count_and_alive_bytes(
        ...
        for (_slot, store) in self.storage.iter() {
            ...
            if let Some(entry) = stored_sizes_and_counts.get(&id) {
                ...
                {
                    let count_and_status = store.count_and_status.write().unwrap();
