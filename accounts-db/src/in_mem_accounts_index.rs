...
struct StartupInfo<T: IndexValue, U: DiskIndexValue + From<T> + Into<T>> {
    ...
    insert: Mutex<Vec<(Pubkey, (Slot, U), usize)>>,
...
impl<T: IndexValue, U: DiskIndexValue + From<T> + Into<T>> InMemAccountsIndex<T, U> {
    ...
    pub fn startup_insert_only(&self, items: &[(Pubkey, (Slot, T), usize)]) {
        ...
        self.stats().copy_us.fetch_add(m.end_as_us(), Ordering::Relaxed);
    }
