...
impl<'a> PackagingOptimizer<'a> {
    ...
    pub fn get_packages(
        &self,
    ) -> Result<Vec<&Vec<&HashedTransaction>>, PackagingOptimizerError> {
        if self.sanity_check() {
            ...
            for cost_aligned_package in &self.packed_packages {
                result.push( cost_aligned_package.package() );
            }
            Ok(result)
        } else {
    ...
    fn add_to_new_package(&mut self, transaction: &'a HashedTransaction<'a>, keys: &[Pubkey], cost: &u32) {
