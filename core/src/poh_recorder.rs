...
impl PohRecorder {
    ...
    pub fn reset_internal(
        ...
        let mut cache = vec![];
        {
            ...
            // didn't seem to make a difference. let _ = self.record_sender.send(Record::new_empty(self.empty_recorder.result_sender.clone())); // poh will not respond on the result channel
        }
        let poh_hash = {
    ...
    pub fn record(
        ...
        self.report_metrics(bank_slot, false);
        loop {
            let now = Instant::now();
