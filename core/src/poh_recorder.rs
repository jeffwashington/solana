...
pub struct PohRecorder {
    ...
    record_us: u64,
    last_metric: Instant,
    sender_mixin: Sender<(Hash, Sender<Option<(PohEntry, u64, Sender<()>)>>)>,
...
impl PohRecorder {
    ...
    pub fn record1(
        ...
        mixin: Hash,
        transactions: Vec<Transaction>,
        sender_result: Sender<Option<(PohEntry, u64, Sender<()>)>>,
    ) -> Result<()> {
        // Entries without transactions are used to track real-time passing in the ledger and
        // cannot be generated by `record()`
        assert!(!transactions.is_empty(), "No transactions provided");
        self.report_metrics(bank_slot);
        //error!("record");
        let result = self.flush_cache(false)?;
        ...
        //error!("Sending mixin");
        if self.sender_mixin.send((mixin, sender_result)).is_err() {
            return Err(PohRecorderError::MaxHeightReached); // TODO error code
        }
        Ok(())
    }
    pub fn record2(
        ...
        mixin: Hash,
        transactions: Vec<Transaction>,
        result: Option<(PohEntry, u64, Sender<()>)>
    ) -> Result<()> {
        self.waits += 1;
        let res = result;
        ...
        //error!("got result");
        if let Some((poh_entry, tick_height, sender)) = res {
            let entry = Entry {
                num_hashes: poh_entry.num_hashes,
                hash: poh_entry.hash,
                transactions,
    ...
    pub fn new_with_clear_signal(
        ...
        Receiver<(Hash, Sender<Option<(PohEntry, u64, Sender<()>)>>)>,
