...
impl<V: IsCached> HybridBTreeMap<V> {
    ...
    /*
    pub fn values(&self) -> Values<K, V2<V>> {
        self.disk.values(self.bin_index)
    }
    */
    ...
    /*
    pub fn entry2(&mut self, key: K) -> Entry<'_, K, V2<V>> {
        self.disk.entry(self.bin_index, key)
    }
    */

    pub fn get(&self, _key: &K) -> Option<&V2<V>> {
        panic!("");
        //None//self.disk.get(key)
    }
