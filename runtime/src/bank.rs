    ...
    fn drop(&mut self) {
        if !self.skip_drop.load(Relaxed) {
            if let Some(drop_callback) = self.drop_callback.read().unwrap().0.as_ref() {
                error!("{} {} drop_callback", file!(), line!());
                drop_callback.callback(self);
            } else {
                error!("{} {} Bank.Drop", file!(), line!());
                // Default case
                // 1. Tests
...
pub(crate) mod tests {
    ...
    fn test_clean_nonrooted() {
        ...
        error!("{} {} test_clean_nonrooted", file!(), line!());
        ...
        error!("{} {} test_clean_nonrooted drop bank1", file!(), line!());
        drop(bank1);
        error!("{} {} test_clean_nonrooted after drop bank1", file!(), line!());
        ...
        bank3.deposit(&pubkey1, some_lamports + 1);
