...
impl AccountsDb {
    ...
    fn combine_ancient_slots(&self, sorted_slots: Vec<Slot>, max_root: Slot) {
        ...
        for slot in sorted_slots {
            if first {
                first = false;
                error!("ancient_append_vec: combine_ancient_slots max_root: {}, first slot: {}, distance from max: {}, storages: {}, # slots to go through: {}", max_root, slot, max_root.saturating_sub(slot), self.storage.map.len(), sorted_slots.len());
            }
        ...
        t.stop();
        error!(
            "ancient_append_vec: done. slots: {:?}, time(ms): {}, storages after: {}",
            len,
            t.as_ms(),
    ...
    */
    ...
    pub fn flush_accounts_cache(&self, force_flush: bool, requested_flush_root: Option<Slot>) {
        ...
        /*
        todo: this causes consensus issues
    ...
    fn flush_rooted_accounts_cache(
        ...
        if false {
            for &root in cached_roots.iter().rev() {
                if self
                    .flush_slot_cache(&[root], should_flush_f.as_mut())
                    .is_some()
                {
                    num_roots_flushed += 1;
                }
            ...
            let combine = std::cmp::min(10000, roots.len());
            error!("writing combined");
            if self
                .flush_slot_cache(&roots[0..combine], should_flush_f.as_mut())
                .is_some()
            {
                num_roots_flushed += std::cmp::min(roots.len(), combine);
            }

            for root in roots.into_iter().skip(combine) {
                error!("writing individual: {}", root);
                // individual
                if self
                    .flush_slot_cache(&[root], should_flush_f.as_mut())
                    .is_some()
        ...
        if !all_accounts.is_empty() {
            {
                ...
                error!(
                    "combining {} slots into 1 slot: {}, {:?}, storages: {:?}, new_slot_orig: {}",
                    slot_caches.len(),
                    new_slot,
            ...
            let mut matched = HashSet::<Pubkey>::default();
            for slot_cache_idx in 0..len {
                let slot_cache_idx = len - slot_cache_idx - 1; // add in reverse order so higher slot contents are after lower slot contents
                ...
                accounts_this_slot.iter().for_each(|key| {
                    if matched.contains(key) {
                        error!("combining key already written: {}", key);
                    }
                    matched.insert(*key);
            ...
            for i in 0..slots.len() {
                ...
                if slots_under_contention.contains(&slot) {
                    ...
                    self.remove_unrooted_slots_synchronization
                        .signal
                        .notify_all();
                    error!("combining failed to lock");
                    return None;
                } else {
        ...
        let flush_stats = Some(vec![{
            ...
            slots.iter().for_each(|slot| {
                if let Some(slot_cache) = self.accounts_cache.slot_cache(*slot) {
                    ...
                    error!("combining no slot cache for: {}", slot);
                }
            });
            if data.len() == 1 {
    ...
    fn calculate_accounts_hash_helper_with_verify(
        ...
        if debug_verify {
            ...
            let success = total_lamports == expected_capitalization.unwrap_or(total_lamports);
            assert!(success, "update_accounts_hash_with_index_option mismatch. hashes: {}; lamports: {}; expected lamports: {:?}, using index: {}, slot: {}, diff: {}", hash, total_lamports, expected_capitalization, use_index, slot, std::cmp::max(expected_capitalization.unwrap().saturating_sub(total_lamports), total_lamports.saturating_sub(expected_capitalization.unwrap())));
        }
