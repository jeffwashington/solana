...
// If this is too big, we have to scan this many storages if any one of these slots
...
impl AccountsDb {
            ...
            + Sync,
    {
        ...
        error!(
            "scanning: {}-{}, {}, first_boundary: {}, chunks: {}",
            range.start, range.end, width, first_boundary, chunks,
        );
        (0..chunks)
            .into_par_iter()
            .map(|chunk| {
    ...
    pub fn generate_index(&self, limit_load_slot_count_from_snapshot: Option<usize>, verify: bool) {
        ...
        for pass in 0..passes {
            let flushers = if pass == 0 {
                let num_threads = 0;// std::cmp::max(2, num_cpus::get() / 2);
                (0..num_threads)
                    .into_iter()
                    .map(|_| {
