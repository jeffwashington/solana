...
impl AccountsDb {
        ...
        I: Iterator<Item = &'a Arc<AccountStorageEntry>> + Clone,
    {
        ...
        let was_ancient = {
            ...
            stores
                .next()
                .map(|store| store.accounts.is_ancient())
                .unwrap_or_default()
                && stores.next().is_none()
        };
        if !was_ancient {
        ...
        if aligned_total > 0 {
            ...
            if was_ancient {
                let is_full_ancient = {
                    ...
                    stores
                        .next()
                        .map(|store| store.accounts.is_full_ancient())
                        .unwrap_or_default()
                        && stores.next().is_none()
                };
        ...
        self.accounts_index.range_scan_accounts(
            ...
            |pubkey, (account_info, slot)| {
                ...
                let account_slot = self
                    .get_account_accessor(slot, pubkey, &account_info.storage_location())
                    .get_loaded_account()
                    .map(|loaded_account| (pubkey, loaded_account.take_account(), slot)); //.unwrap();
                if account_slot.is_none() {
                    panic!(
                        "missing: {:?}, index: {:?}",
                        (slot, pubkey, &account_info.storage_location()),
                        {
        ...
        loop {
            ...
            if new_slot == slot && new_storage_location.is_store_id_equal(&storage_location) {
                ...
                error!(
                    ...
                    self.accounts_index
                        .get_account_read_entry(pubkey)
                        .map(|entry| entry.slot_list().clone())
                );
                                ...
                                if !account_info.is_zero_lamport() {
                                    ...
                                    .and_then(
                                        |loaded_account| {
                                            ...
                                            if false && check_hash && !self.is_filler_account(pubkey) {  // this will not be supported anymore
                                                let computed_hash =
    ...
    */
    ...
    fn maybe_rehash_rewrites(
        ...
        let mut use_stored = true;
        let expected = ExpectedRentCollection::new(pubkey, loaded_account, storage_slot, epoch_schedule, rent_collector, max_slot_in_storages_exclusive, find_next_slot, filler_account_suffix);
        if expected.is_none() {
            return loaded_account.loaded_hash();
        }
        use_stored = false;
    ...
    fn scan_snapshot_stores_with_cache(
        ...
        let find_next_slot = |slot: Slot| {
            let roots = self.find_next_slot(slot, &accounts_cache_and_ancestors.map(|(_, ancestors, _)| ancestors));
                if roots.is_none() {
                    error!("not next slot: {}, max: {}", slot, self.accounts_index.max_root_inclusive());
                }
            roots
        };
    ...
    pub fn store_cached<'a, T: ReadableAccount + Sync + ZeroLamport>(&self, accounts: impl StorableAccounts<'a, T>) {
        self.store(accounts, self.caching_enabled);
    }
