...
pub struct AccountsDB {
    ...
    pub exe_cache: RwLock<HashMap<Pubkey, AccountNoData>>,
...
impl Default for AccountsDB {
    fn default() -> Self {
        ...
        AccountsDB {
            ...
            exe_cache: RwLock::new(HashMap::default()),//<solana_sdk::pubkey::Pubkey, AccountNoData>::new()),
        }
    }
}
...
pub mod tests {
    ...
    fn sample_storages_and_accounts() -> (SnapshotStorages, Vec<CalculateHashIntermediate>) {
        ...
        accounts.store_uncached(
            ...
            &[(
                &pubkey0,
                &AccountNoData::new(raw_expected[0].lamports, 1, &Account::default().owner),
            )],
        );
        accounts.store_uncached(
            SLOT,
            &[(&pubkey127, &AccountNoData::new(128, 1, &Account::default().owner))],
        );
        accounts.store_uncached(
            SLOT,
            &[(&pubkey128, &AccountNoData::new(129, 1, &Account::default().owner))],
        );
        accounts.store_uncached(
            SLOT,
            &[(&pubkey255, &AccountNoData::new(256, 1, &Account::default().owner))],
        );
    ...
    fn test_accountsdb_scan_account_storage_no_bank() {
        ...
        let acc = AccountNoData::new(1, 48, &Account::default().owner);
        let sm = StoredMeta {
            data_len: 1,
            pubkey,
            write_version: 1,
    ...
    fn test_accountsdb_add_root() {
        ...
        assert_eq!(db.load_slow_no_data(&ancestors, &key), Some((account0, 0)));
    }
    ...
    fn test_accountsdb_latest_ancestor() {
        ...
        let ancestors = vec![(1, 1)].into_iter().collect();
        assert_eq!(&db.load_slow_no_data(&ancestors, &key).unwrap().0, &account1);
    ...
    fn test_accountsdb_root_one_slot() {
        ...
        assert_eq!(&db.load_slow_no_data(&ancestors, &key).unwrap().0, &account0);

        db.add_root(0);

        let ancestors = vec![(1, 1)].into_iter().collect();
        assert_eq!(db.load_slow_no_data(&ancestors, &key), Some((account1, 1)));
        let ancestors = vec![(2, 2)].into_iter().collect();
        assert_eq!(db.load_slow_no_data(&ancestors, &key), Some((account0, 0))); // original value
    }
    ...
    fn test_accountsdb_add_root_many() {
        ...
        for _ in 1..100 {
            ...
            let account = db.load_slow_no_data(&ancestors, &pubkeys[idx]).unwrap();
        ...
        for _ in 1..100 {
            let idx = thread_rng().gen_range(0, 99);
            let ancestors = vec![(0, 0)].into_iter().collect();
            let account0 = db.load_slow_no_data(&ancestors, &pubkeys[idx]).unwrap();
            let ancestors = vec![(1, 1)].into_iter().collect();
            let account1 = db.load_slow_no_data(&ancestors, &pubkeys[idx]).unwrap();
            let default_account = AccountNoData {
                lamports: (idx + 1) as u64,
                ..AccountNoData::default()
            };
    ...
    fn test_accounts_unsquashed() {
        ...
        // original account
        let ancestors = vec![(0, 0), (1, 1)].into_iter().collect();
        assert_eq!(db0.load_slow_no_data(&ancestors, &key), Some((account1, 1)));
        let ancestors = vec![(0, 0)].into_iter().collect();
        assert_eq!(db0.load_slow_no_data(&ancestors, &key), Some((account0, 0)));
    }
    ...
    fn test_remove_unrooted_slot() {
        ...
        assert!(db.load_slow_no_data(&ancestors, &key).is_none());
    ...
    fn test_remove_unrooted_slot_snapshot() {
        ...
        assert!(db.load_slow_no_data(&unrooted_slot_ancestors, &key).is_none());
    }
    ...
    fn create_account(
        ...
        for t in 0..num {
            ...
            pubkeys.push(pubkey);
            assert!(accounts.load_slow_no_data(&ancestors, &pubkey).is_none());
    ...
    fn update_accounts(accounts: &AccountsDB, pubkeys: &[Pubkey], slot: Slot, range: usize) {
        for _ in 1..1000 {
            ...
            let ancestors = vec![(slot, 0)].into_iter().collect();
            if let Some((mut account, _)) = accounts.load_slow_no_data(&ancestors, &pubkeys[idx]) {
                account.lamports += 1;
                accounts.store_uncached(slot, &[(&pubkeys[idx], &account)]);
                if account.lamports == 0 {
                    let ancestors = vec![(slot, 0)].into_iter().collect();
                    assert!(accounts.load_slow_no_data(&ancestors, &pubkeys[idx]).is_none());
                } else {
                    let default_account = AccountNoData {
                        lamports: account.lamports,
                        ..AccountNoData::default()
    ...
    fn check_accounts(
        ...
        for _ in 0..num {
            ...
            let account = accounts.load_slow_no_data(&ancestors, &pubkeys[idx]);
            let account1 = Some((
    ...
    fn test_account_one() {
        ...
        let ancestors = vec![(0, 0)].into_iter().collect();
        let account = db.load_slow_no_data(&ancestors, &pubkeys[0]).unwrap();
        let default_account = AccountNoData {
            lamports: 1,
            ..AccountNoData::default()
        };
    ...
    fn test_account_grow_many() {
        ...
        for (i, key) in keys.iter().enumerate() {
            assert_eq!(
                accounts.load_slow_no_data(&ancestors, &key).unwrap().0.lamports,
                (i as u64) + 1
            );
        }
    ...
    fn test_account_grow() {
        ...
        let ancestors = vec![(0, 0)].into_iter().collect();
        assert_eq!(
            accounts.load_slow_no_data(&ancestors, &pubkey1).unwrap().0,
            account1
        );
        assert_eq!(
            accounts.load_slow_no_data(&ancestors, &pubkey2).unwrap().0,
            account2
        );
    ...
    fn test_lazy_gc_slot() {
        ...
        //new value is there
        let ancestors = vec![(1, 1)].into_iter().collect();
        assert_eq!(accounts.load_slow_no_data(&ancestors, &pubkey), Some((account, 1)));
    }
    ...
    fn assert_load_account(
        ...
        let ancestors = vec![(slot, 0)].into_iter().collect();
        let (account, slot) = accounts.load_slow_no_data(&ancestors, &pubkey).unwrap();
                ...
                std::thread::Builder::new()
                    ...
                    .spawn(move || {
                        ...
                        loop {
                            ...
                            let (account, slot) =
                                db.load_slow_no_data(&HashMap::new(), &pubkey).unwrap_or_else(|| {
    ...
    fn test_accountsdb_scan_accounts() {
        ...
        let accounts: Vec<AccountNoData> =
            db.unchecked_scan_accounts("", &ancestors, |accounts: &mut Vec<AccountNoData>, option| {
                accounts.push(option.1.account_no_data());
            });
    ...
    fn test_cleanup_key_not_removed() {
        ...
        assert_eq!(db.load_slow_no_data(&ancestors, &key1).unwrap().0.lamports, 3);
    }
    ...
    fn test_store_large_account() {
        ...
        let ancestors = vec![(0, 0)].into_iter().collect();
        let ret = db.load_slow_no_data(&ancestors, &key).unwrap();
    ...
    fn test_hash_frozen_account_data() {
        ...
        // AccountNoData data may not be modified
        let mut account_modified = account.clone();
    ...
    fn test_hash_stored_account() {
        ...
        assert_eq!(
            ...
            expected_account_hash,
            "AccountNoData-based hashing must be consistent with StoredAccountMeta-based one."
        );
    }
    ...
    fn test_bank_hash_stats() {
        ...
        let mut account = db.load_slow_no_data(&ancestors, &key).unwrap().0;
        account.lamports -= 1;
    ...
    fn test_store_reuse() {
        ...
        for (key, account_ref) in keys[..num_to_store].iter().zip(account_refs) {
            assert_eq!(accounts.load_slow_no_data(&ancestors, key).unwrap().0, account_ref);
        }
    }
    ...
    fn test_zero_lamport_new_root_not_cleaned() {
        ...
        assert_eq!(
            db.load_slow_no_data(&HashMap::new(), &account_key),
    ...
    fn test_store_load_cached() {
        ...
        assert!(db.load_slow_no_data(&HashMap::new(), &key).is_none());
        ...
        let ancestors = vec![(slot, 1)].into_iter().collect();
        assert_eq!(
            db.load_slow_no_data(&ancestors, &key),
            Some((account0.clone(), slot))
        );

        // Adding root will return the account even without ancestors
        db.add_root(slot);
        assert_eq!(db.load_slow_no_data(&HashMap::new(), &key), Some((account0, slot)));
    }
    ...
    fn test_flush_accounts_cache() {
        ...
        assert_eq!(
            db.load_slow_no_data(&ancestors, &unrooted_key),
            Some((account0.clone(), unrooted_slot))
        );
        db.flush_accounts_cache(true, None);

        // After the flush, the unrooted slot is still in the cache
        assert!(db.load_slow_no_data(&ancestors, &unrooted_key).is_some());
        assert!(db
            .accounts_index
        ...
        assert_eq!(
            db.load_slow_no_data(&HashMap::new(), &key5),
            Some((account0.clone(), root5))
        );
        assert_eq!(
            db.load_slow_no_data(&HashMap::new(), &key6),
            Some((account0, root6))
        );
    }
    ...
    fn setup_scan(
        ...
        let t_scan = Builder::new()
            ...
            .spawn(move || {
                db.scan_accounts(
                    &scan_ancestors,
                    |_collector: &mut Vec<(Pubkey, AccountNoData)>, maybe_account: Option<(&Pubkey, AccountNoData, Slot)>| {
