    ...
    */
              ...
              for (slot, iter_items) in slot_caches {
                  ...
                  .filter_map(|iter_item| {
                      ...
                      if should_flush {
                          ...
                          total_size += (account.data().len() + STORE_META_OVERHEAD) as u64;
                          num_flushed += 1;
                          Some(((*key, account.clone(), *slot), hash))
                      } else {
                          // If we don't flush, we have to remove the entry from the
                          // index, since it's equivalent to purging
                          purged_slot_pubkeys.insert((*slot, *key));
    ...
    >(
        ...
        let accounts_and_meta_to_store: Vec<_> = accounts
            .iter()
            .map(|(pubkey, account, original_slot)| {
                self.read_only_accounts_cache
                    .remove(**pubkey, *original_slot);
                ...
                .map(|pubkeys| {
                    ...
                    let result: Vec<Hash> = pubkeys
                        ...
                        .filter_map(|pubkey| {
                            ...
                            let mut interesting =         pubkey == &Pubkey::from_str("5qa1JHr7JgnZuX6zWNdZsVVwLEsbP7LEqfAb2vxfksr5").unwrap();
    ...
    fn maybe_rehash_rewrites(
        ...
        */
        ...
        let interesting = true && (pubkey
            == &Pubkey::from_str("5qa1JHr7JgnZuX6zWNdZsVVwLEsbP7LEqfAb2vxfksr5").unwrap()
        ...
        */
        ...
        /*
        if pubkey == &Pubkey::from_str("5qa1JHr7JgnZuX6zWNdZsVVwLEsbP7LEqfAb2vxfksr5").unwrap() {
        }
        */
        assert!(!force_rehash);
            ...
            .map(|(infos_chunk, accounts_chunk)| {
                ...
                for (info, pubkey_account) in infos_chunk.iter().zip(accounts_chunk.iter()) {
                    ...
                    let mut interesting = pubkey
                        == &Pubkey::from_str("5qa1JHr7JgnZuX6zWNdZsVVwLEsbP7LEqfAb2vxfksr5")
                            .unwrap();
                    if interesting {
                        error!(
                            "account update: {}, {}, {}, {:?}",
