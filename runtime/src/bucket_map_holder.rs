...
pub const DEFAULT_AGE_INCREMENT: u8 = 15; // effectively = slots
...
impl<V: IsCached> BucketMapHolder<V> {
    ...
    pub fn bg_flusher(&self, exit: Arc<AtomicBool>) {
            ...
            < self.get_desired_threads()
        {
            ...
            //error!("thread starting asleep: {}, desired: {}, threads: {}", self.stats.active_flush_threads.load(Ordering::Relaxed), self.get_desired_threads(), self.num_threads);
            false
        };
    ...
    // return true means put thread to sleep
    fn check_throughput(&self, can_put_thread_to_sleep: bool) -> bool {
            ...
            .elapsed(THROUGHPUT_POLL_MS, true)
        {
            ...
            let log = |increase| {
            info!(
                "throughput: bins scanned: {}, elapsed: {}ms, {}, desired threads: {}, primary thread idle: {}, can put to sleep: {}, active thrads: {}, increase: {}",
                bins_scanned,
                elapsed_ms,
                ratio,
                self.get_desired_threads(),
            ...
            if can_put_thread_to_sleep
                && (ratio > FULL_FLUSHES_PER_1000_S || primary_thread_was_idle || bins_scanned == 0)
            {
                // decrease
    ...
    fn flush_upsert_or_update(
        ...
        // probably not here either...v.set_insert(false);
        1
    }
    ...
    fn flush_internal(&self, ix: usize, startup: bool, age: Age) -> bool {
        ...
        for (k, v) in read_lock.iter() {
            ...
            if keep_this_in_cache {
                ...
                had_dirty = true;
                if dirty {
                    v.set_dirty(true); // restore dirty since we removed it earlier
                }
            }
        ...
        let m2 = Measure::start("flush_purge");
        {
            ...
            for key in delete_keys.into_iter() {
                ...
                if let HashMapEntry::Occupied(occupied) = wc.entry(key) {
                    ...
                    if !startup {
                        //error!("removing2: {}, {:?}", key, occupied.get().slot_list.read().unwrap());
                    }
        ...
        if TRY_READ_LOCKS_FIRST {
            ...
            // try read lock first
            {
                ...
                if let Some(occupied) = res {
                    ...
                    //error!("upsert read lock: {}, {:?}", key, new_value);
                    self.upsert_item_in_cache(
                        occupied,
                        new_value,
                        reclaims,
        ...
        match res {
            HashMapEntry::Occupied(occupied) => {
                ...
                self.get_caching(ix, item, key); // updates from disk if necessary
                                                 // already in cache, so call update_static function
                                                 //error!("upsert write lock: {}, {:?}", key, new_value);
                                                 self.upsert_item_in_cache(
                    item,
                    new_value,
                    reclaims,
            ...
            HashMapEntry::Vacant(vacant) => {
                ...
                if let Some(current) = r {
                    ...
                    //error!("upsert write lock vacant: {}, {:?}, old disk: {:?}", key, new_value, current);
                    let addref = WriteAccountMapEntry::update_slot_list(
                        &mut slot_list,
                        slot,
                        new_entry,
    ...
    pub fn get_no_cache(&self, key: &Pubkey) -> Option<V2<V>> {
        ...
        if r.is_some() {
            //error!("disk, get: {}, {:?}", key, r.as_ref().unwrap().1);
        }
