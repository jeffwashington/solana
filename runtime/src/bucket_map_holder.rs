...
//#[derive(Debug)]
/*
pub struct WriteCacheEntry<V: IsCached> {
    pub data: V2<V>,
    pub age: u8,
    pub dirty: bool,
    pub insert: bool,
...
impl<V: IsCached> BucketMapWriteHolder<V> {
    ...
    pub fn entry<'a>(&self, lock: &'a mut CacheWriteLock<V>, key: K) -> Entry<'a, K, V2<V>> {
        // first, do lookup TODO
        error!("FIX THIS!!!!!!!!!!!!!");
        lock.entry(key)
    }
    ...
    fn merge_slot_lists<'a>(
        correct_entry: &'a AccountMapEntry<V>,
        cache_entry: &'a WriteCacheEntry<V>,
    ) {
        let previous_slot_entry_was_cached = false;
        let mut _reclaims = Vec::default();
        let slot_list_cache = cache_entry.slot_list.write().unwrap();
        let mut slot_list_correct = correct_entry.slot_list.write().unwrap();
        for (slot, new_entry) in slot_list_cache.iter() {
            let addref = WriteAccountMapEntry::update_slot_list(
                &mut slot_list_correct,
                *slot,
                new_entry.clone(),
                &mut _reclaims,
                previous_slot_entry_was_cached,
            ...
            if addref {
                correct_entry.add_un_ref(true);
            }
        }
        //cache_entry.ref_count()
        panic!(""); // race conditions here - if someone else already has a ref to the arc, it is difficult to make the refcounts work out right
    }
    ...
    pub fn get_no_cache(&self, key: &Pubkey) -> Option<V2<V>> {
        ...
        let r = r.map(|(ref_count, slot_list)| {
            Arc::new(AccountMapEntryInner {
                ref_count: AtomicU64::new(ref_count),
    ...
    fn get_caching<'b>(&self, instance: &'b WriteCacheEntryArc<V>, key: &Pubkey) -> Option<()> {
        instance.set_age(self.set_age_to_future());
        ...
        if instance.must_do_lookup_from_disk() {
            ...
            if let Some(v) = r {
                Self::merge_slot_lists(&v, &instance);
                instance.set_must_do_lookup_from_disk(false);
                panic!("copy data over here");
                // return;
            }
            // else, we now know we were the only info that exists for this account, so we have all we need
            instance.set_must_do_lookup_from_disk(false);
            instance.set_confirmed_not_on_disk(true);
        }
        Some(())
    }
    ...
    {
        let mut _m = Measure::start("");
        let _r = if !self.in_mem_only {
            self.flush(ix, false, None);
