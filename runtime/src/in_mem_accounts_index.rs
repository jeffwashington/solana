...
impl<T: IndexValue> InMemAccountsIndex<T> {
    ...
    /// scan loop
    /// holds read lock
    fn flush_scan(&self, current_age: Age, startup: bool) -> FlushScanResult<T> {
    ...
    fn flush_internal(&self, _flush_guard: &FlushGuard) {
        ...
        // may have to loop if disk has to grow and we have to restart
        {
            ...
            let FlushScanResult {
                ...
                evictions_random,
                dirty_items,
            } = self.flush_scan(current_age, startup);
            {
    ...
    fn evict_from_cache(
        ...
        for k in evictions {
            if let Entry::Occupied(occupied) = map.entry(k) {
                ...
                // all conditions for evicting succeeded, so really evict item from in-mem cache
                evicted += 1;
                occupied.remove();
            }
        }
