...
impl<T: IndexValue> InMemAccountsIndex<T> {
    ...
    pub fn get2(&self, pubkey: &K) -> Option<AccountMapEntry<T>> {
        panic!("adding to in mem");
    ...
    /// Wueue up these insertions for when the flush thread is dealing with this bin.
    ...
    fn write_startup_info_to_disk(&self) {
        ...
        assert!(self.map_internal.read().unwrap().is_empty(), "len: {}, first: {:?}", self.map_internal.read().unwrap().len(), self.map_internal.read().unwrap().iter().take(1).collect::<Vec<_>>());
        ...
        insert.into_iter().for_each(|(slot, k, v)| {
            let entry = (slot, v);
            let new_ref_count = 1; // writing to disk, not cached - ref_count is always 1 in this case
