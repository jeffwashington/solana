...
impl<'a> From<&'a AccountNoData> for AccountMeta {
    fn from(account: &'a AccountNoData) -> Self {
        Self {
...
impl<'a> StoredAccountMeta<'a> {
    ...
    pub fn clone_account(&self) -> AccountNoData {
        AccountNoData {
    ...
    fn sanitize_lamports(&self) -> bool {
        // Sanitize 0 lamports to ensure to be same as AccountNoData::default()
        self.account_meta.lamports != 0 || self.clone_account() == AccountNoData::default()
    }
...
impl AppendVec {
    ...
    pub fn get_account_test(&self, offset: usize) -> Option<(StoredMeta, AccountNoData)> {
    ...
    pub fn append_accounts(
        &self,
        accounts: &[(StoredMeta, &AccountNoData)],
        hashes: &[Hash],
    ) -> Vec<usize> {
        let _lock = self.append_lock.lock().unwrap();
...
pub mod test_utils {
    ...
    pub fn create_test_account(sample: usize) -> (StoredMeta, AccountNoData) {
        let data_len = sample % 256;
        let mut account = AccountNoData::new(sample as u64, 0, &Pubkey::default());
...
pub mod tests {
    ...
    impl AppendVec {
        fn append_account_test(&self, data: &(StoredMeta, AccountNoData)) -> Option<usize> {
    ...
    fn test_new_from_file_crafted_zero_lamport_account() {
        ...
        let data_len = 3_u64;
        let mut account = AccountNoData::new(0, data_len as usize, &owner);
