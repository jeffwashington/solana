...
impl AncientSlotInfos {
    ...
    fn clear_should_shrink_after_threshold(&mut self, percent_of_shrunk_data: u64) {
        ...
        // todo - figure out how many bytes we'd save. if it's too few, don't do it.
...
impl AccountsDb {
    ...
    fn combine_ancient_slots_packed(&self, slots: Vec<Slot>, tuning: PackedAncientStorageTuning) {
        ...
        use log::*;
        //error!("infos: {ancient_slot_infos:?}");
    ...
    fn sort_and_filter_ancient_slots(
        &self,
        ancient_slot_infos: &mut AncientSlotInfos,
        ...
        */
        ...
        ancient_slot_infos.clear_should_shrink_after_threshold(tuning.percent_of_shrunk_data);
        use log::*;
        //error!("infos after ratio: {ancient_slot_infos:?}");
        ancient_slot_infos.filter_by_smallest_capacity(tuning.max_ancient_slots);
        use log::*;
        //error!("infos after capacity: {ancient_slot_infos:?}");
    }
}
...
pub mod tests {
    ...
    fn test_calc_ancient_slot_info_one_shrink_one_not() {
        ...
        for slot1_shrink in [false, true] {
            ...
            use log::*;error!("shrinks {shrinks:?}, all infos: {:?}", infos.all_infos.iter().map(|r| r.storage.slot()).collect::<Vec<_>>());
            storages
                .iter()
                .zip(infos.all_infos.iter())
                .for_each(|(storage, info)| {
