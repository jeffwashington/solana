use crate::{
    ...
    contains::Contains,
    hybrid_btree_map::{
        HybridBTreeMap, HybridEntry as Entry,
    },
...
pub const ITER_BATCH_SIZE: usize = 2;
...
impl<T: IsCached> WriteAccountMapEntry<T> {
    ...
    // true if item existed already and was updated
    pub fn upsert<'a>(
...
impl<T: IsCached> AccountsIndex<T> {
    ...
    where
        F: FnMut(&Pubkey, (&T, Slot), Option<ReadAccountMapEntry<'a, T>>),
        ...
        R: RangeBounds<Pubkey> + Debug,
    {
        ...
        for pubkey_list in self.iter(range) {
            ...
            for pubkey in pubkey_list {
                //error!("pubkey: {}", pubkey);
                num_keys_iterated += 1;
                let lock = ReadAccountMapEntry::new_with_lock(pubkey, self.get_account_maps_read_lock(&pubkey));
                match lock {
                    AccountIndexGetResultInternal::Found(item) => {
                        let ix = self.latest_slot(Some(ancestors), &item.slot_list()[..], max_root);
                        if let Some(ix) = ix {
                            let result = item.slot_list()[ix];

                            let mut load_account_timer = Measure::start("load_account");
                            //let list_item = &slot_list[index];
                                func(&pubkey, (&result.1, result.0), Some(item));
    ...
    fn slot_list_mut<RT>(
        ...
        let mut read_lock = self.account_maps[self.bin_calculator.bin_from_pubkey(pubkey)]
            .write()
            .unwrap();
    ...
    pub(crate) fn scan_accounts<'a, F>(
        &'a self,
        ancestors: &Ancestors,
        scan_bank_id: BankId,
        func: F,
    ...
    pub(crate) fn unchecked_scan_accounts<'a, F>(
        &'a self,
        metric_name: &'static str,
        ancestors: &Ancestors,
        func: F,
    ...
    pub(crate) fn range_scan_accounts<'a, F, R>(
        &'a self,
        metric_name: &'static str,
        ancestors: &Ancestors,
        range: R,
    ...
    /// call func with every pubkey and index visible from a given set of ancestors
    pub(crate) fn index_scan_accounts<'a, F>(
        &'a self,
        ancestors: &Ancestors,
        scan_bank_id: BankId,
        index_key: IndexKey,
            ...
            .map(|(pubkey_bin, items)| {
                ...
                items
                    ...
                    .for_each(|(pubkey, new_item, is_zero_lamport)| {
                        let already_exists = WriteAccountMapEntry::upsert(
                            &w_account_maps,
                            &pubkey,
                            new_item,
                            &mut _reclaims,
                        ...
                        if already_exists || is_zero_lamport {
...
pub mod tests {
    ...
    fn test_get_empty() {
        ...
        let mut num = 0;
        index.unchecked_scan_accounts("", &ancestors, |_pubkey, _index, _| num += 1);
        assert_eq!(num, 0);
    }
    ...
    fn test_insert_with_ancestors() {
        ...
        index.unchecked_scan_accounts("", &ancestors, |pubkey, _index, _| {
            if pubkey == key {
                found_key = true
            };
            num += 1
    ...
    fn run_test_range(
        ...
        index.range_scan_accounts("", &ancestors, pubkey_range, |pubkey, _index, _| {
    ...
    fn test_update_gc_purged_slot() {
        ...
        let mut num = 0;
        let mut found_key = false;
        index.unchecked_scan_accounts("", &Ancestors::default(), |pubkey, _index, _| {
            if pubkey == key {
                found_key = true;
