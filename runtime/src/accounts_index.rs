...
pub const BINS_FOR_TESTING: usize = 8;
pub const BINS_FOR_BENCHMARKS: usize = 8;
...
impl<T: IsCached> AccountsIndex<T> {
    ...
    fn allocate_accounts_index(
        mut config: Option<AccountsIndexConfig>,
    ) -> (
        LockMapType<T>,
        ...
        let threads = config.as_ref().and_then(|config| config.threads);
        let drives = config.as_mut().and_then(|config| config.drives.take());
        ...
        let bucket_map = InMemAccountsIndex::new_bucket_map(bins, max_search, threads, drives);
