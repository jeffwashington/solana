...
pub trait IsCached: 'static + Clone + Debug + PartialEq + ZeroLamport + Sync + Send {
...
#[self_referencing]
pub struct WriteAccountMapEntry<'a, 'b: 'a, T: 'static + Clone + IsCached + std::fmt::Debug> {
...
impl<'a, 'b: 'a, T: IsCached> WriteAccountMapEntry<'a, 'b, T> {
    ...
    pub fn update(&mut self, slot: Slot, account_info: T, reclaims: &mut SlotList<T>) {
        let addref = !account_info.is_cached();
        self.slot_list_mut(|list, refcount| {
            let addref2 = Self::update_slot_list(list, slot, account_info, reclaims, false);
            if addref || addref2 {
                *refcount += 1;
            }
        });
...
impl<T: Clone + std::fmt::Debug + IsCached> Drop for AccountsIndex<T> {
    fn drop(&mut self) {
...
pub mod tests {
    ...
    fn test_insert_with_lock_no_ancestors() {
        ...
        assert_eq!((slot, account_info), new_entry.slot_list[0]);
