...
impl AccountsHash {
    ...
    fn de_dup_and_eliminate_zeros<'a>(
        &self,
        binned_data: &'a [Vec<&'a [CalculateHashIntermediate]>],
        stats: &mut HashStats,
        max_bin: usize,
    ) -> (Vec<Vec<&'a Hash>>, u64) {
        ...
        let hashes: Vec<Vec<&Hash>> = (0..max_bin)
            ...
            .map(|bin| {
                let (hashes, lamports_bin, unreduced_entries_count) =
                    self.de_dup_accounts_in_parallel(binned_data, bin);
                {
    ...
    fn get_item<'a, 'b>(
        ...
        pubkey_division: &'b [Vec<&[CalculateHashIntermediate]>],
        indexes: &'a mut [usize],
    ...
    fn de_dup_accounts_in_parallel<'a>(
        &self,
        pubkey_division: &'a [Vec<&'a [CalculateHashIntermediate]>],
        pubkey_bin: usize,
...
pub mod tests {
    ...
    fn for_rest(original: &[CalculateHashIntermediate]) -> Vec<Vec<&[CalculateHashIntermediate]>> {
        vec![vec![original]]
    }
    ...
    fn empty_data() -> Vec<Vec<&'static [CalculateHashIntermediate]>> {
        vec![vec![&EMPTY_DATA]]
    }
    ...
    fn test_de_dup_accounts_in_parallel<'a>(
        account_maps: &'a [Vec<&'a [CalculateHashIntermediate]>],
    ) -> (Vec<&'a Hash>, u64, usize) {
