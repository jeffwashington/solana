...
pub struct HashStats {
    ...
    pub count_pubkey_bin_searches: AtomicU64,
    ...
    pub fn log(&self) {
        datapoint_info!(
            ...
            (
                ...
                self.count_pubkey_bin_searches.load(Ordering::Relaxed),
                i64
            ),
            (
...
impl AccountsHasher {
    ...
    fn find_first_pubkey_in_bin(
        ...
        stats
            .count_pubkey_bin_searches
            .fetch_add(searches, Ordering::Relaxed);
...
pub mod tests {
    ...
    fn for_rest(original: &[CalculateHashIntermediate]) -> SortedDataByPubkey<'_> {
        vec![original]
    }
