...
impl AccountsHash {
    ...
    fn de_dup_and_eliminate_zeros(
        sorted_data_by_pubkey: Vec<Vec<Vec<CalculateHashIntermediate2>>>,
        stats: &mut HashStats,
        range: &Range<usize>,
    ) -> (Vec<Vec<Hash>>, u64) {
        ...
        error!("range: {}, {}", range.start, range.end);
    ...
    fn de_dup_accounts_in_parallel(
        ...
        while !first_items.is_empty() {
            ...
            if item.lamports != ZERO_RAW_LAMPORTS_SENTINEL {
                ...
                if true {//last_slot == item.slot {
                    //error!("jwash: {:?}", CalculateHashIntermediate2::from(item.clone()));
                }
                hashes.push(item.hash);
            }else {
                if true {//last_slot == item.slot {
                    //error!("jwash: {:?} skipping", item);
                }
            }
            index += 1;
            if index >= bin.len() {
    ...
    */
    ...
    fn test_accountsdb_lamport_overflow() {
        ...
        let input = vec![
            ...
            CalculateHashIntermediate2::new(Hash::new_unique(), offset + 1, Pubkey::new_unique(), 0),
        ];
