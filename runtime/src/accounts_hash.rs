...
impl AccountsHash {
    ...
    fn de_dup_and_eliminate_zeros<'a>(
        &self,
        sorted_data_by_pubkey: &'a Vec<Vec<Vec<CalculateHashIntermediate>>>,
        stats: &mut HashStats,
        max_bin: usize,
    ) -> (Vec<Vec<&'a Hash>>, u64) {
    ...
    pub fn rest_of_hash_calculation(
        ...
        if !previous_state.remaining_unhashed.is_empty() {
            ...
            hashes.insert(0, _remaining_unhashed.as_ref().unwrap().into_iter().collect::<Vec<_>>());
        ...
        if no_progress {
            ...
            hashes.into_iter().for_each(|v| {
                if !v.is_empty() {
                    next_pass.reduced_hashes.push(v.into_iter().cloned().collect());
                }
            });
        }
...
pub mod tests {
    ...
    fn test_accountsdb_de_dup_accounts_zero_chunks() {
        ...
        let (hashes, lamports, _) = AccountsHash::default()
            .de_dup_accounts_in_parallel(&vec, 0);
