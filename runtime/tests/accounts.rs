...
use solana_sdk::{account::AccountNoData, clock::Slot, pubkey::Pubkey};
        ...
        for current_slot in 0..100 {
            while alive_accounts.len() <= 10 {
                alive_accounts.push((
                    ...
                    AccountNoData::new(thread_rng().gen_range(0, 50), 0, &owner),
                ));
            }
...
fn test_bad_bank_hash() {
    ...
    let mut accounts_keys: Vec<_> = (0..max_accounts)
        ...
        .map(|_| {
            ...
            let account = AccountNoData::new(lamports, some_data_len, &key);
            (key, account)
        })
        .collect();
