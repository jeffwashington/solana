    ...
    F: Fn(&Accounts, &[Pubkey]) + Send + Copy,
{
    ...
    let new_accounts: Vec<_> = (0..num_new_keys)
        .map(|_| AccountNoData::new(1, 0, &AccountNoData::default().owner))
        .collect();
    bencher.iter(|| {
...
fn bench_concurrent_scan_write(bencher: &mut Bencher) {
    store_accounts_with_possible_contention("concurrent_scan_write", bencher, |accounts, _| loop {
        test::black_box(accounts.load_by_program(&HashMap::new(), &AccountNoData::default().owner));
    })
}
...
fn setup_bench_dashmap_iter() -> (Arc<Accounts>, DashMap<Pubkey, (AccountNoData, Hash)>) {
    let accounts = Arc::new(Accounts::new_with_config(
        vec![
            PathBuf::from(std::env::var("FARF_DIR").unwrap_or_else(|_| "farf".to_string()))
    ...
    for _ in 0..num_keys {
        dashmap.insert(
            Pubkey::new_unique(),
            (
                AccountNoData::new(1, 0, &AccountNoData::default().owner),
                Hash::new_unique(),
            ),
        );
    }
