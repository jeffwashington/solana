...
fn test_program_bpf_duplicate_accounts() {
    ...
    for program in programs.iter() {
        ...
        let payee_account = AccountNoData::new(10, 1, &program_id);
...
fn test_program_bpf_invoke_sanity() {
    ...
    for program in programs.iter() {
        ...
        let account = AccountNoData::new(42, 100, &invoke_program_id);
        bank.store_account(&argument_keypair.pubkey(), &account);

        let invoked_argument_keypair = Keypair::new();
        let account = AccountNoData::new(10, 10, &invoked_program_id);
        bank.store_account(&invoked_argument_keypair.pubkey(), &account);

        let from_keypair = Keypair::new();
        let account = AccountNoData::new(84, 0, &solana_sdk::system_program::id());
        ...
        let account = AccountNoData::new(84, 0, &solana_sdk::system_program::id());
        bank.store_account(&from_keypair.pubkey(), &account);
        bank.store_account(&derived_key1, &AccountNoData::default());
        let instruction = Instruction::new_with_bytes(
            invoke_program_id,
            &[
...
fn test_program_bpf_program_id_spoofing() {
    ...
    let account = AccountNoData::new(10, 0, &solana_sdk::system_program::id());
    bank.store_account(&from_pubkey, &account);

    let to_pubkey = Pubkey::new_unique();
    let account = AccountNoData::new(0, 0, &solana_sdk::system_program::id());
...
fn assert_instruction_count() {
    ...
    for program in programs.iter() {
        ...
        let mut account = RefCell::new(AccountNoData::default());
