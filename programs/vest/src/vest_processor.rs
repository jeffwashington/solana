...
mod tests {
    ...
    fn test_verify_account_unauthorized() {
        ...
        let account = AccountNoData::new_ref(1, 0, &solana_config_program::id());
    ...
    fn test_verify_date_account_incorrect_program_id() {
        ...
        let date_pubkey = solana_sdk::pubkey::new_rand();
        let account = AccountNoData::new_ref(1, 0, &id()); // <-- Attack! Pass Vest account where Config account is expected.
        let keyed_account = KeyedAccount::new(&date_pubkey, false, &account);
        assert_eq!(
    ...
    fn test_verify_date_account_uninitialized_config() {
        ...
        let date_pubkey = solana_sdk::pubkey::new_rand();
        let account = AccountNoData::new_ref(1, 0, &solana_config_program::id()); // <-- Attack! Zero space.
        let keyed_account = KeyedAccount::new(&date_pubkey, false, &account);
        assert_eq!(
            verify_date_account(&keyed_account, &date_pubkey).unwrap_err(),
    ...
    fn test_verify_date_account_invalid_date_config() {
        ...
        let date_pubkey = solana_sdk::pubkey::new_rand();
        let account = AccountNoData::new_ref(1, 1, &solana_config_program::id()); // Attack! 1 byte, enough to sneak by `get_config_data()`, but not DateConfig deserialize.
        let keyed_account = KeyedAccount::new(&date_pubkey, false, &account);
        assert_eq!(
            verify_date_account(&keyed_account, &date_pubkey).unwrap_err(),
            InstructionError::InvalidAccountData
